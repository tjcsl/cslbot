...
def get_default(nick, session, send, config, source):
    ...
    if location is None:
        try:
            ...
            if hostip:
                ...
                db_file = resource_filename(
                    Requirement.parse("CslBot"), join("cslbot", config["db"]["geoip"])
                )
                location = get_zipcode(db_file, hostip)
                if location is not None:
                    send(
                        "No default location for %s, GeoIP guesses that your zip code is %s."
                        % (nick, location)
                    )
                    return location
        ...
        # default to TJHSST
        send(
            "No default location for %s and unable to guess a location, defaulting to TJ (22312)."
            % nick
        )
        return "22312"
    else:
...
def valid_location(location, apikey):
    data = get(
        "http://api.wunderground.com/api/%s/conditions/q/%s.json" % (apikey, location)
    ).json()
...
def get_weather(cmdargs, send, apikey):
    if cmdargs.string.startswith("-"):
        data = get(
            "http://api.wunderground.com/api/%s/conditions/q/%s.json" % (apikey, cmdargs.string[1:])
        ).json()
        if "current_observation" in data:
            data = {
                "display_location": {"full": cmdargs.string[1:]},
                "weather": "Sunny",
                "temp_f": "94.8",
                "feelslike_f": "92.6",
        ...
        try:
            data = get(
                "http://api.wunderground.com/api/%s/conditions/q/%s.json" % (apikey, cmdargs.string)
            ).json()
            forecastdata = get(
                "http://api.wunderground.com/api/%s/forecast/q/%s.json" % (apikey, cmdargs.string)
            ).json()
            alertdata = get(
                "http://api.wunderground.com/api/%s/alerts/q/%s.json" % (apikey, cmdargs.string)
            ).json()
        ...
        if "current_observation" in data:
            ...
            send(
                "{} results found, please be more specific: {}".format(
                    len(results), ", ".join(names)
                )
            )
            return False
        else:
...
def get_forecast(cmdargs, send, apikey):
    forecastdata = get(
        "http://api.wunderground.com/api/%s/forecast10day/q/%s.json" % (apikey, cmdargs.string)
    ).json()
    if "forecast" in forecastdata:
    ...
    for day in forecastdata:
        if (
            ...
            == (cmdargs.date.day, cmdargs.date.month, cmdargs.date.year)
        ):
            forecast = "%s, High: %s, Low: %s" % (
                day["conditions"], day["high"]["fahrenheit"], day["low"]["fahrenheit"]
            )
            send(
                "Forecast for %s on %s: %s"
                % (cmdargs.string, cmdargs.date.strftime("%x"), forecast)
            )
            return
...
def get_hourly(cmdargs, send, apikey):
    forecastdata = get(
        "http://api.wunderground.com/api/%s/hourly10day/q/%s.json" % (apikey, cmdargs.string)
    ).json()
    ...
    for hour in forecastdata:
        ...
        if date == (cmdargs.hour, cmdargs.date.day, cmdargs.date.month, cmdargs.date.year):
            ...
            send(
                "Forecast for %s on %s at %s00: %s"
                % (cmdargs.string, cmdargs.date.strftime("%x"), cmdargs.hour, forecast)
            )
            return
    send(
        "Couldn't find data for %s hour %s in the 10-day hourly forecast"
        % (cmdargs.date.strftime("%x"), cmdargs.hour)
    )
