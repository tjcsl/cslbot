...
def cmd(send, msg, args):
    ...
    spaces = ['"Still Alive" on piano', 'polo shirt', 'workstation broken', 'man man', 'csssuf on deadtom',
              ...
              'jwoglom\'s sweatpants', 'jwoglom\'s chicken wrap', 'smash bros', 'anya hates someone', 'fwilson doesn\'t mod MC']
...
def getLongestInColumn(array, column):
    ...
    for i in range(1, len(array)):
        if len(array[i][column]) > len(array[maxlen][column]):
            maxlen = i
    return len(array[maxlen][column])
