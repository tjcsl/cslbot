...
from cslbot.helpers.orm import Poll_responses, Polls, Quotes, Scores, Urls  # noqa
...
def get_polls(session: Session) -> Dict[int, str]:
    rows = session.query(Polls).filter(Polls.deleted == 0, Polls.active == 1).order_by(
        Polls.id
    ).all()
...
def get_responses(session: Session, polls: Dict[int, str]) -> Dict[int, Dict[str, List[str]]]:
    ...
    for pid in polls.keys():
        ...
        rows = session.query(Poll_responses).filter(Poll_responses.pid == pid).order_by(
            Poll_responses.response
        ).all()
        for row in rows:
...
def get_winners(
    polls: Dict[int, str], responses: Dict[int, Dict[str, List[str]]]
) -> Dict[int, str]:
    winners = {}
    for pid in polls.keys():
