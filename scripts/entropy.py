...
def main(confdir="/etc/cslbot"):
    ...
    channel = '#tjhsst'
    users = session.query(Log.source).filter(Log.target == channel, or_(Log.type == 'privmsg', Log.type == 'pubmsg', Log.type == 'action')).having(func.count(Log.id) > 500).group_by(Log.source).all()
    freq = []
    for user in users:
